...
fil = open('document_sentences_test.txt','w')
...
def tf_idf_sim(claim, lines,freqs=None):
    ...
    fil.write(json.dumps(ret_lines)+'\n')
...
def tf_idf_claim(line):
    if 'predicted_pages' in line:
        ...
        pages = [p[0] for p in sorted_p_new[:args.max_page]]
        print(pages)
...
if __name__ == "__main__":
    ...
    jlr = JSONLineReader()
    with open(args.in_file,"r") as f, open("data/fever/{0}.sentences3.{3}.p{1}.s{2}.jsonl".format(args.split, args.max_page, args.max_sent,"precomputed" if args.use_precomputed else "not_precomputed"), "w+") as out_file:
        lines = jlr.process(f)
